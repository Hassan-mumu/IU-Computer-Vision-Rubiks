...
def detect_face_colors(frame):
    ...
    for i in range(3):
        ...
        for j in range(3):
            ...
            cell = frame[y+round(cell_size/3):y+round(cell_size/3*2), x+round(cell_size/3):x+round(cell_size/3*2)]
...
def main():
    ...
    canvas = np.ones((Constante.HEIGHT_CANVAS, Constante.WIDTH_CANVAS, 3), dtype=np.uint8) * 30  # dark screen 
    ...
    print(kociemba.solve(cubeTableToString(cube_faces)))
